...
void LLFloaterWater::onSaveNotecard()
{
	...
	{
		LLNotificationsUtil::add("WLSaveNotecardAlert", LLSD(), LLSD(), boost::bind(&LLFloaterWater::saveNotecardCallback, this, _1, _2));
	}
	else
	{
		...
		LLPointer<LLFloaterWaterNotecardCreatedCallback> cb = new LLFloaterWaterNotecardCreatedCallback();
...
void LLFloaterWaterNotecardCreatedCallback::fire(const LLUUID& inv_item)
{
	LLWaterParamManager * param_mgr = LLWaterParamManager::instance();
